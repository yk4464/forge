...
⚡ 一个用 Rust 写的、流式优先的终端编程 Agent（v0.0.1 原型）。
...
- **流式事件接口** — 正文、推理内容和工具事件通过统一的 `AgentEvent` 接口驱动 Ratatui TUI；shell 增量输出转发待 S0 修复
...
- **shell 工具** — Git Bash 执行命令、超时控制（默认 120s / 上限 600s）和结果截断；实时输出转发与捕获内存上限仍有已知缺口，列入 S0
...
详见 [ROADMAP.md](ROADMAP.md)：包含完整功能清单、依赖、实现顺序和阶段验收，不承诺日历工期。v0.0.1 已有核心循环、TUI、三协议面、SQLite 与上下文压缩基础，已知缺口从 S0 开始修复。

- [ ] **S0 修复基线**：shell 流式输出与内存上限、异常结束、压缩消息完整性
- [ ] **S1 可控运行与数据可靠性**：取消、原始记录保真、故障恢复、任务预算、运行接口
